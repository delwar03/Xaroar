...
int main(int argc, char* argv[]) {
    ...
    // Call your lexer function to tokenize the input
    Token* tokens = lexer_function(input);

    // Call the parser function to create the syntax tree
    Node* syntax_tree = parser(tokens);

    // Printing the syntax tree
    printf("Syntax Tree:\n");
    print_tree(syntax_tree, 0, "root");

    // Free allocated memory for tokens and syntax tree
    // Add functions to free tokens and syntax tree nodes
